...
🔍 如何识别用户偏好并传递参数：
1. **地域偏好**：如果用户提到省份/城市名称，请提取到 preferences.locations 数组
   - 例子："江苏或浙江" → locations: ["江苏", "浙江"]
   - 例子："上海的大学" → locations: ["上海"]
   - 例子："省内" → locations: [用户所在省份]

2. **专业偏好**：如果用户提到具体专业名称，请提取到 preferences.majors 数组
   - 例子："学习自动化" → majors: ["自动化"]
   - 例子："计算机或软件工程" → majors: ["计算机", "软件工程"]
   - 例子："人工智能相关" → majors: ["人工智能", "智能科学"]

3. **院校类型**：如果用户提到985/211/双一流，请提取到 preferences.collegeTypes 数组
   - 例子："985院校" → collegeTypes: ["985"]
   - 例子："211或双一流" → collegeTypes: ["211", "双一流"]
