...
export class RecommendationEngine {
   ...
   */
  private async filterCandidateColleges(userPrefs: UserPreferences): Promise<CoreCollege[]> {
    const collegeRepo = AppDataSource.getRepository(CoreCollege);
    const admissionScoreRepo = AppDataSource.getRepository(CoreAdmissionScore);
   ...
   * 按专业组分组
   */
  private groupByMajorGroup(plans: CoreEnrollmentPlan[]): Record<string, CoreEnrollmentPlan[]> {
    const groups: Record<string, CoreEnrollmentPlan[]> = {};
  ...
  private async calculateVolunteerScore(
    ...
    // 获取院校和专业详细信息
    const college = await AppDataSource.getRepository(CoreCollege)
  ...
  private async calculateMajorScore(
    ...
    const majorRepo = AppDataSource.getRepository(CoreMajor);
    let score = 0;
    const maxScore = 100;
