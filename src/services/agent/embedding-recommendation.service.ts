...
export class EmbeddingEnhancedRecommendationService {
  ...
  private async getCandidates(userInfo: {
    ...
    const latestYearResult = await AppDataSource.getRepository(CoreEnrollmentPlan)
    ...
    // 查询招生计划（兼容多种格式）
    const enrollmentPlans = await AppDataSource.getRepository(CoreEnrollmentPlan)
      .createQueryBuilder('ep')
    ...
    if (enrollmentPlans.length === 0) {
      ...
      // Core Layer uses redundant design, no college relation
      const withoutCollegeName = enrollmentPlans.filter(p => !p.collegeName).length;
      if (withoutCollegeName > 0) {
        console.warn(`⚠️  有 ${withoutCollegeName} 条记录的collegeName为空`);
      }
    }
    ...
    for (const plan of enrollmentPlans) {
      // Core Layer uses redundant design - collegeName is directly in the plan
      const collegeName = plan.collegeName;
  ...
  private async scoreCandidate(
    ...
    if (!major && plan.majorGroupCode) {
      ...
      if (majorsInGroup.length > 0) {
        ...
        let bestMatch: { major: CoreMajor; score: number } | null = null;
   ...
   * 计算就业得分
   */
  private calculateEmploymentScore(major: CoreMajor | null, preferences: AgentPreference[]): number {
    if (!major) {
      // 没有major对象时，返回中等分数
      return 60;
    }
    ...
    if (major.careerFields && major.careerFields.length > 0) {
      ...
      if (targetIndustries.length > 0) {
        const matchedCount = major.careerFields.filter((field: any) =>
   ...
   * 计算性格匹配度
   */
  private calculatePersonalityFit(major: CoreMajor, preferences: AgentPreference[]): number {
    let score = 70; // 基础分
   ...
   * 计算职业目标匹配度
   */
  private calculateCareerAlignment(major: CoreMajor, preferences: AgentPreference[]): number {
    let score = 0;
    let totalWeight = 0;
    ...
    if (major.careerFields && targetIndustries.length > 0) {
      ...
      for (const industry of targetIndustries) {
        if (major.careerFields.some((field: any) => field.includes(industry) || industry.includes(field))) {
          matchCount++;
        }
      }
